...
export default function Montures() {
  ...
  const fetchMontures = async () => {
    setIsLoading(true);
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/stocks/montures/`);
  ...
  const handleAdd = async () => {
    ...
    try {
      if (editingId) {
        await fetch(`${import.meta.env.VITE_API_URL}/api/stocks/montures/${editingId}/`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(payload)
        });
      } else {
        await fetch(`${import.meta.env.VITE_API_URL}/api/stocks/montures/`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(payload)
        });
                  ...
                  <td style={{ padding: '1rem 1.5rem', textAlign: 'right', display: 'flex', gap: '0.5rem', justifyContent: 'flex-end', alignItems: 'center' }}>
                    ...
                    <button onClick={async (e) => { 
                      ...
                      if (window.confirm('Êtes-vous sûr de vouloir supprimer cette monture ?')) {
                        try {
                          await fetch(`${import.meta.env.VITE_API_URL}/api/stocks/montures/${item.id}/`, { method: 'DELETE' });
                          fetchMontures();
                        } catch(err) {
